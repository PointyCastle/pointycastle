...
void _registerAsymmetricBlockCiphers() {
  AsymmetricBlockCipher.registry
      .registerDynamicFactory(_pkcs1AsymmetricBlockCipherFactory);
...
void _registerMacs() {
  ...
  Mac.registry["RIPEMD-128/HMAC"] =
      (_) => new HMac(new Digest("RIPEMD-128"), 64);
  Mac.registry["RIPEMD-160/HMAC"] =
      (_) => new HMac(new Digest("RIPEMD-160"), 64);
...
void _registerModesOfOperation() {
  BlockCipher.registry.registerDynamicFactory((algorithmName) =>
      _modeOfOperationFactory(algorithmName, "CBC",
  ...
  BlockCipher.registry.registerDynamicFactory((algorithmName) =>
      _modeOfOperationFactory(
          algorithmName,
          "CTR",
          (underlyingCipher) => new StreamCipherAsBlockCipher(
              underlyingCipher.blockSize,
              new CTRStreamCipher(underlyingCipher))));
  BlockCipher.registry.registerDynamicFactory((algorithmName) =>
      _modeOfOperationFactory(algorithmName, "ECB",
          (underlyingCipher) => new ECBBlockCipher(underlyingCipher)));
  BlockCipher.registry.registerDynamicFactory((algorithmName) =>
      _modeOfOperationFactory(algorithmName, "GCTR",
...
void _registerSecureRandoms() {
  ...
  SecureRandom.registry
      .registerDynamicFactory(_ctrAutoSeedPrngSecureRandomFactory);
}

void _registerSigners() {
  Signer.registry.registerDynamicFactory(_ecdsaSignerFactory);
  Signer.registry["MD2/RSA"] =
      (_) => new RSASigner(new Digest("MD2"), "06082a864886f70d0202");
  Signer.registry["MD4/RSA"] =
      (_) => new RSASigner(new Digest("MD4"), "06082a864886f70d0204");
  Signer.registry["MD5/RSA"] =
      (_) => new RSASigner(new Digest("MD5"), "06082a864886f70d0205");
  ...
  Signer.registry["SHA-1/RSA"] =
      (_) => new RSASigner(new Digest("SHA-1"), "06052b0e03021a");
...
BlockCipher _modeOfOperationFactory(String algorithmName, String modeName,
  ...
  var underlyingCipher =
      _createOrNull(() => new BlockCipher(algorithmName.substring(0, sep)));
  if (underlyingCipher != null) {
...
BlockCipher _variableSizeModeOfOperationFactory(
  ...
  var blockSizeInBits =
      int.parse(algorithmName.substring(sep + 1 + modeName.length + 1));
