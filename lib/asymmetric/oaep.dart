...
Uint8List _arraycopy(Uint8List src, int srcPos, Uint8List dest, int destPos, int length) {
...
// Copies the objects of iterable, skipping skipCount objects first, into the range start, inclusive, to end, exclusive, of the list. [...]
    ...
    dest.setRange(destPos, destPos + length , src.sublist(srcPos, srcPos + length));
    return dest;
  }
 ...
 */
  Uint8List _maskGeneratorFunction1(
    ...
    if ((counter * hashBuf.length) < length) {
      ...
      hashBuf = _arraycopy(hashBuf, 0, mask, counter * hashBuf.length,mask.length - (counter * hashBuf.length));
    }
    return mask;
  }
}
